...
def print_tracking_results(tracking_results):
    for i in range(len(tracking_results)):
        track = tracking_results[i]
        print(f"Track ID: {track['id']}, Bbox: ({track['bbox_x']},{track['bbox_y']},{track['bbox_w']},{track['bbox_h']}), "
              f"Age: {track['age']}, Conf: {track['confidence']}")
...
def main():
    ...
    filtered_events_buf = ActivityNoiseFilterAlgorithm.get_empty_output_buffer()  # Added buffer for filtered events
    ...
    def process_tracking(evs):
        if len(evs) != 0:
            ...
            print_tracking_results(tracking_results)  # Print tracking results
        ...
        for evs in mv_iterator:
            ...
            if args.activity_time_ths > 0:
                ...
                if args.activity_trail_ths > 0:
                    trail_filter.process_events(events_buf, filtered_events_buf)  # Corrected call

                process_tracking(filtered_events_buf.numpy())

            elif args.activity_trail_ths > 0:
                trail_filter.process_events(evs, filtered_events_buf)
                process_tracking(filtered_events_buf.numpy())
            else:
